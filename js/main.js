...
(function() {
  ...
  createSubstitutionAlert = function(substituteAtArray, mayNotReplace) {
    var body, check_with, j, l, len, len1, m, minute, r, range, ranges, result, title;
    ranges = [];
    r = 0;
    for (j = 0, len = substituteAtArray.length; j < len; j++) {
      minute = substituteAtArray[j];
      if (!ranges[r]) {
        ranges[r] = [];
    ...
    result = [];
    for (m = 0, len1 = ranges.length; m < len1; m++) {
      range = ranges[m];
  ...
  $(FORM_ID).validate({
    ...
    invalidHandler: function(form, validator) {
      var error, errors, j, len, message, ref;
...
<table class="table table-striped table-condensed table-staminia table-staminia-contributions">
  ...
  // Fill Form Helper
  fillForm = function() {
    var $field, field, fields, i, j, len, params, paramsString;
...
${description_message}`);
  ...
  updateCHPPPlayerFields = function() {
    var PlayersData, index, j, len, mc, name, number, optionElement, player, select, selectP1, selectP2;
    PlayersData = Staminia.Teams[$("#CHPP_Team").val()].PlayersData;
    if (PlayersData == null) {
      return;
    }
    ...
    for (index = j = 0, len = PlayersData.length; j < len; index = ++j) {
  ...
  setupCHPPPlayerFields = function(checkUrlParameter = false) {
    var Teams, index, j, len, optionElement, select, team;
    Teams = Staminia.Teams;
    if ((Teams == null) || Teams.length === 0) {
      return;
    }
    select = $(document.createElement("select"));
    for (index = j = 0, len = Teams.length; j < len; index = ++j) {
      team = Teams[index];
      optionElement = $(document.createElement("option"));
      optionElement.attr("value", index);
  ...
  setPlayerFormFields = function(player, checkUrlParameter = false) {
