...
(() => {
  ...
  $.extend(Staminia.CONFIG, {
    ...
    DEBUG_STEP: 1,
    AUTOSTART: true,
    PREDICTIONS_ANDREAC: [[0.5036, 0.231, 0, 0, 0, 0], [0, 0.3492, 0.118, 0, 0, 0], [0, 0.2514, 0.159, 0, 0, 0], [0, 0.3546, 0.0825, 0.0556, 0, 0], [0, 0.3236, 0.078, 0.1086, 0, 0], [0, 0.248, 0.108, 0.1375, 0, 0], [0, 0.344, 0.031, 0.0688, 0, 0], [0, 0.3256, 0.078, 0.0604, 0, 0], [0, 0.1302, 0.468, 0, 0.1149, 0], [0, 0.0733, 0.442, 0, 0.1508, 0], [0, 0.2039, 0.442, 0, 0.076, 0], [0, 0.1383, 0.413, 0.1073, 0.1071, 0], [0, 0.1314, 0.218, 0.1848, 0.0669, 0], [0, 0.0652, 0.183, 0.2081, 0.0803, 0], [0, 0.1831, 0.183, 0.1556, 0.0484, 0], [0, 0.1341, 0.276, 0.135, 0.0671, 0], [0, 0, 0, 0.0808, 0.1306, 0.3077], [0, 0, 0.195, 0.055, 0.2189, 0.1778], [0, 0, 0.195, 0.055, 0.2661, 0.1778], [0, 0, 0, 0.0901, 0.1334, 0.2441]],
    PREDICTIONS_HO: [[0.4897, 0.231, 0, 0, 0, 0], [0, 0.3492, 0.114, 0, 0, 0], [0, 0.253, 0.154, 0, 0, 0], [0, 0.3488, 0.0825, 0.0556, 0, 0], [0, 0.3283, 0.078, 0.1086, 0, 0], [0, 0.2582, 0.108, 0.1375, 0, 0], [0, 0.355, 0.031, 0.0688, 0, 0], [0, 0.3214, 0.078, 0.0604, 0, 0], [0, 0.1348, 0.468, 0, 0.1148, 0], [0, 0.0727, 0.442, 0, 0.1475, 0], [0, 0.1974, 0.442, 0, 0.076, 0], [0, 0.1383, 0.413, 0.1073, 0.1063, 0], [0, 0.1314, 0.213, 0.1873, 0.0669, 0], [0, 0.0629, 0.178, 0.2193, 0.0814, 0], [0, 0.1769, 0.178, 0.1585, 0.0484, 0], [0, 0.1245, 0.269, 0.1235, 0.0596, 0], [0, 0, 0, 0.079, 0.1297, 0.3046], [0, 0, 0.201, 0.0545, 0.2214, 0.1781], [0, 0, 0.201, 0.0545, 0.2519, 0.1781], [0, 0, 0, 0.115, 0.1323, 0.2632]],
    PR_ENUM_SKILL: {
      Keeper: 0,
      Defending: 1,
      Playmaking: 2,
  ...
  $(FORM_ID).validate({
    ...
    submitHandler: (form) => {
      ...
      if (isVerboseModeEnabled()) {
        let tempHTML = `<h3 class="legend-like">${Staminia.messages.strength_table}</h3><table class="table table-striped table-condensed table-staminia table-staminia-strength width-auto"><thead><tr><th></th><th>${Staminia.messages.player1}</th><th>${Staminia.messages.player2}</th></tr></thead><tbody><tr><td>${Staminia.messages.strength}</td><td>${number_format(result.player1Strength, 2)}</td><td>${number_format(result.player2Strength, 2)}</td></tr><tr><td>${Staminia.messages.strength_st_independent}</td><td>${number_format(result.player1StrengthStaminaIndependent, 2)}</td><td>${number_format(result.player2StrengthStaminaIndependent, 2)}</td></tr></tbody></table><p><small>${Staminia.messages.used_in_calculation}</small></p>`;
        $("#tabContributions").append(tempHTML);
        const tableHeader = `<thead><tr><th class="min-width">${Staminia.messages.substitution_minute}</th><th>${Staminia.messages.total_contribution}</th><th>${Staminia.messages.contribution_percent}</th><th>${Staminia.messages.p1_contrib}</th><th>${Staminia.messages.p2_contrib}</th><th>${Staminia.messages.notes}</th></tr></thead>`;
        tempHTML = `<h3 class="legend-like">${Staminia.messages.contribution_table}</h3><table class="table table-striped table-condensed table-staminia table-staminia-contributions">` + tableHeader + "<tbody>";
        ...
        for (const minute in result.minutes) {
          ...
          const note = (isMax ? "MAX" : isMin ? "MIN" : 100 - percentContribution < 1 ? "~ 1%" : "") + (minute === player1LowStamina ? " " + Staminia.messages.p1_low_stamina : "") + (minute === player2LowStamina ? " " + Staminia.messages.p2_low_stamina : "");
          const css_classes = (isMax ? " max" : "") + (isMin ? " min" : "");
          tempHTML += `<tr class="${css_classes}"><td>${minute}</td><td>${totalContribution}</td><td>${percentContribution}%</td><td>${p1Contribution}</td><td>${p2Contribution}</td><td>${note}</td></tr>`;
        }
  ...
  const createAlert = (params) => `<div class="alert alert-block alert-${params.type} fade in" id="${params.id}"><button class="close" data-dismiss="alert" type="button">&times;</button><h4 class="alert-heading">${params.title}</h4><p id="${params.id}Body">${params.body}</p></div>`;
