...
(() => {
  ...
  const getContribution = (minute, stamina, startsAtMinute, pressing) => {
    ...
    const pressingDecay = pressing ? 55e-5 * (9 - engineStamina) : 0;
    const decay = Math.max(0.0325, -39e-4 * engineStamina + 0.0633 + pressingDecay);
    const rest = 0.1875;
    if (engineStamina > 8) {
  ...
  const calculateStrength = (skill, form, stamina, experience, include_stamina) => {
    ...
    if (Staminia.CONFIG.DEBUG) {
      const tempHTML = `strength(skill = <b>${skill}</b>, form = <b>${form}</b>, stamina = <b>${stamina}</b>, experience = <b>${experience}</b>, include_stamina = <b>${include_stamina}</b>)<br/>&nbsp;&nbsp;c_form(<b>${form}</b>) = <b>${c_form}</b><br/>&nbsp;&nbsp;c_stamina(<b>${stamina}</b>) = <b>${c_stamina}</b><br/>&nbsp;&nbsp;c_experience(<b>${experience}</b>) = <b>${c_experience}</b><br/>&nbsp;&nbsp;result_w_stamina = <b>${skill * c_form * c_stamina * c_experience}</b><br/>&nbsp;&nbsp;result_wo_stamina = <b>${skill * c_form * 1 * c_experience}</b><br/>&nbsp;&nbsp;result = <b>${result}</b><br/><br/>`;
  ...
  const getAdvancedSkill = (player) => {
    ...
    if (Staminia.CONFIG.DEBUG) {
      ...
      const tempHTML = `getAdvancedSkill(player = <b>${player}</b>)<br/>&nbsp;&nbsp;Position: <b>${PR_ENUM_ROLE[position]}</b><br/>&nbsp;&nbsp;Keeper: <b>${Staminia.number_format(keeper_coeff * 100, 2)}</b>% * <b>${keeper}</b><br/>&nbsp;&nbsp;Defending: <b>${Staminia.number_format(defending_coeff * 100, 2)}</b>% * <b>${defending}</b><br/>&nbsp;&nbsp;Playmaking: <b>${Staminia.number_format(playmaking_coeff * 100, 2)}</b>% * <b>${playmaking}</b><br/>&nbsp;&nbsp;Winger: <b>${Staminia.number_format(winger_coeff * 100, 2)}</b>% * <b>${winger}</b><br/>&nbsp;&nbsp;Passing: <b>${Staminia.number_format(passing_coeff * 100, 2)}</b>% * <b>${passing}</b><br/>&nbsp;&nbsp;Scoring: <b>${Staminia.number_format(scoring_coeff * 100, 2)}</b>% * <b>${scoring}</b><br/>Expected (if all skills = 1.00): <b>${debug_coeff}</b><br/>Calculated: <b>${total}</b><br/>Match: <b>${total === debug_coeff}</b><br/><br/>`;
      $("#tabDebug").append(tempHTML);
    }
    return total;
  };
  ...
  const debugTableHeader = (header) => `<table class="table table-striped table-bordered table-condensed table-staminia table-staminia-debug width-auto">
  <thead>
    <tr>
      <th colspan="10">
  ...
  <tbody>`;
  const debugTableAddCell = (content, bad_stamina_minute) => `<td ${bad_stamina_minute ? 'style="background: #e0cccc"' : ""}>${content}</td>`;
