...
(function() {
  ...
  getAvgAt90 = function(stamina) {
    var checkpoint, currentEnergy, decay, i, initialEnergy, rest, totalEnergy;
    if (stamina >= 9) {
      return 1;
    }
    totalEnergy = 0;
    ...
    rest = 0.1875;
    for (checkpoint = i = 1; i <= 18; checkpoint = ++i) {
...
Match: <b>${total === debug_coeff}</b><br/><br/>`;
  ...
  Staminia.Engine.start = function() {
    var formReference, i, j, k, l, m, max, mayNotReplace, min, minute, n, o, p1LowStaminaRisk, p1PlayedMinutes, p1_minute, p2LowStaminaRisk, p2PlayedMinutes, p2_minute, player1AVGArray, player1CurrentContribution, player1Experience, player1Form, player1LowStamina, player1Skill, player1Stamina, player1Strength, player1StrengthStaminaIndependent, player1TotalContribution, player2AVGArray, player2CurrentContribution, player2Experience, player2Form, player2LowStamina, player2Skill, player2Stamina, player2Strength, player2StrengthStaminaIndependent, player2TotalContribution, plotDataPartial, plotDataTotal, plotIndex, pressing, ref, ref1, ref10, ref11, ref12, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, secondHalfMax, secondHalfMin, substituteAt, substituteAtSecondHalf, totalContributionArray;
    this.result = {
      minutes: [],
      substituteAt: [],
      substituteAtSecondHalf: [],
    ...
    player1AVGArray = [];
    player2AVGArray = [];
    for (p1_minute = i = ref = KICKOFF, ref1 = FULLTIME; (ref <= ref1 ? i <= ref1 : i >= ref1); p1_minute = ref <= ref1 ? ++i : --i) {
      ...
      player2TotalContribution = 0;
      for (p2_minute = j = 0, ref2 = FULLTIME - p1_minute; (0 <= ref2 ? j < ref2 : j > ref2); p2_minute = 0 <= ref2 ? ++j : --j) {
    ...
    secondHalfMin = +2e308;
    totalContributionArray = [];
    for (minute = k = ref3 = KICKOFF, ref4 = FULLTIME; (ref3 <= ref4 ? k <= ref4 : k >= ref4); minute = ref3 <= ref4 ? ++k : --k) {
      if (!(minute !== HALFTIME)) {
        continue;
      }
    ...
    if (Staminia.isVerboseModeEnabled()) {
      for (minute = l = ref5 = KICKOFF, ref6 = FULLTIME; (ref5 <= ref6 ? l <= ref6 : l >= ref6); minute = ref5 <= ref6 ? ++l : --l) {
        if (!(minute !== HALFTIME)) {
          continue;
        }
        p1PlayedMinutes = minute - 1;
    ...
    for (minute = m = ref7 = KICKOFF, ref8 = FULLTIME; (ref7 <= ref8 ? m <= ref8 : m >= ref8); minute = ref7 <= ref8 ? ++m : --m) {
      if (minute !== HALFTIME) {
        if (totalContributionArray[minute] === max) {
          if (minute === FULLTIME) {
            mayNotReplace = true;
    ...
    substituteAtSecondHalf = [];
    for (minute = n = ref9 = HALFTIME + 1, ref10 = FULLTIME; (ref9 <= ref10 ? n <= ref10 : n >= ref10); minute = ref9 <= ref10 ? ++n : --n) {
    ...
    if (Staminia.isChartsEnabled()) {
      ...
      plotDataPartial[1] = [];
      plotIndex = 0;
      for (minute = o = ref11 = KICKOFF, ref12 = FULLTIME; (ref11 <= ref12 ? o < ref12 : o > ref12); minute = ref11 <= ref12 ? ++o : --o) {
        if (!(minute !== HALFTIME)) {
          continue;
        }
  ...
  printContributionTables = function(pressing = false) {
    var currentContribution, delta, i, j, minute, ref, ref1, ref2, stamina_level, tempAVGHTML, tempHTML, totalContributionArray, totalContributionPressingArray;
    ...
    for (minute = i = ref = KICKOFF, ref1 = FULLTIME, ref2 = Staminia.CONFIG.DEBUG_STEP; ref2 !== 0 && (ref2 > 0 ? i <= ref1 : i >= ref1); minute = i += ref2) {
      tempHTML += `<tr><td><b>${minute}</b></td>`;
      tempAVGHTML += `<tr><td><b>${minute}</b></td>`;
      for (stamina_level = j = 1; j <= 9; stamina_level = ++j) {
